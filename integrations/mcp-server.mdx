...
<Steps>
  <Step title="Copy your auth token">
    In the MemoryPlugin dashboard, click **Connect** in the sidebar and open the **Access** tab. Under **Authentication Token**, click **Copy** to copy **Your Auth Token**. Direct link: [memoryplugin.com/dashboard/connect?tab=access](https://memoryplugin.com/dashboard/connect?tab=access).

    <Frame caption="Connect → Access: copy Your Auth Token, or regenerate it if it ever leaks.">
      <img src="/images/screenshots/tools/connect-access-token.png" alt="The Connect page with the Access tab active, showing the Authentication Token field with Copy and Regenerate buttons" />
    </Frame>
...
The server exposes 13 tools, grouped by feature. Each tool ships with a description that tells the AI when and how to use it, so most of the time the client calls the right one on its own.
...
| `export_conversation` | Download a past conversation as a JSON file. Returns a temporary download link that expires in 15 minutes. |
...
<AccordionGroup>
  ...
  <Accordion title="Auth token invalid">
    Recopy the token from **Connect → Access** and make sure you pasted the whole string. If in doubt, click **Regenerate** and update your config with the new token.
  </Accordion>
