...
- **Sync Status**: Check if memories are up to date
...
The browser extension supports Smart Mode on compatible platforms, which provides enhanced memory management:

<Info>
Smart Mode automatically organizes memories into intelligent categories and provides more contextual memory retrieval.
</Info>

### Features
- **Automatic Categorization**: Memories are automatically organized into relevant categories
- **Contextual Retrieval**: Only relevant memories are injected based on conversation context
- **Category Management**: View and switch between different memory categories
- **Enhanced Instructions**: AI receives more sophisticated instructions for memory handling

### Supported Platforms for Smart Mode
Most major platforms support Smart Mode, including ChatGPT, Claude, Gemini, Perplexity, and others. The extension automatically detects platform capabilities.
...
### Data Handling
- **Local Processing**: The extension processes memories locally before sending to servers
- **Secure Transmission**: All data is transmitted over HTTPS
- **Authentication**: Requires login to access your memories
- **No Auto-Collection**: Memories are only added when you explicitly activate MemoryPlugin
