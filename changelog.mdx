...
<Update label="August 2026" description="Claude's New Export Format, ChatGPT Capture Fix, Smoother MCP Connections" tags={["Features", "Improvements"]}>
  ...
  Imports also email you now: one message when processing starts and one when it finishes, with any problems listed by file. Start a big import and close the tab.

  ### ChatGPT memory capture fixed, and this class of breakage now gets fixed remotely (Extension v4.2.1 and v4.2.2)
  In early August ChatGPT changed the structure of its conversation page, which quietly stopped the extension from seeing new replies and the save commands inside them. v4.2.1 fixes capture so it follows the conversation as it grows. v4.2.2 goes further: every supported platform's conversation container can now be repaired remotely when a site redesigns, so the fix ships the moment it's ready instead of waiting on an extension store review. Open tabs pick it up within about an hour, no reload needed.

  ### Smoother MCP connections
  MCP clients can now discover the remote server before you sign in, and the default authorization scope includes memory writes, so a freshly connected tool can save memories right away instead of hitting a permissions error. Every tool also carries a proper display title, so tool lists in Claude, Cursor, and other clients read like features instead of function names.
...
<Update label="July 2026" description="MemoryPlugin Sync (Desktop App), Quality Mode Rebuilt, a Real Chat Reader, Voice Memories, Free Tools Hub" tags={["Major Release", "Features", "Improvements"]}>
  ...
  The app moved fast after its July 8 launch. By month's end it had grown an embedded version of the full dashboard (memories, Ask, chat history, and imports without opening a browser) and a global **⌘⇧K** search that works from any app on your Mac and searches your entire synced history, ChatGPT and Claude chats included, not just coding tools.
  ...
  - **Project names on coding chats** - conversations synced from Claude Code, Codex, and Cursor carry their project name, with a Project filter in chat history.
