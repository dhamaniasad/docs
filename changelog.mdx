...
<Update label="May 2026" description="Memory Editing & Versioning, Selection-to-Memory Pill, Extension Overhaul" tags={["Features", "Improvements", "API"]}>
  ...
  Highlight any text inside a chat message on ChatGPT, Claude, Gemini, Grok, DeepSeek, Kimi, MiniMax, or TypingMind and a glassmorphic **Extract memory** pill appears next to your selection. One click runs an AI extraction pass, produces a clean atomic memory, and saves it to your current bucket. Works with double-click word, triple-click paragraph, and arbitrary drag selections.
  ...
  Recall is now smarter and quieter. Instead of querying your chat history on every message you send, the AI decides when context from past conversations would help and crafts targeted queries. Up to 15 queries can run in parallel for complex topics. A frosted-glass preview panel shows what's being searched and lets you preview or cancel the injection before it's sent.

  The old auto/ask mode selector has been retired — recall now fires only when the AI requests it.

  ### Extension v3.4.0 — Direct Supabase integration & panel redesign
  ...
  - **Avatar pill in panel header** — your profile picture appears at the top of the panel and opens your account view.
  ...
  - **Lighter bundle** — content script size dropped by nearly 50% on every page load.
  ...
  ### Live dashboard
  Memory list updates on the dashboard are now powered by Supabase Realtime. Edit a memory from the extension, MCP, or the API and the dashboard reflects the change instantly — no more polling delay.
  ...
  - **`contentType` filter** on the memory search API — narrow results to images or text only
  - **Image-only and text-only search modes** — skip the unrelated pipeline entirely for faster, more focused results
  - **Dense-only image search** with score-based merging for better relevance
  ...
  - **TypingMind chat history ingestion** no longer 500s on large imports
  - **Image memory deletion** now fully removes the image from storage and clears it from search
  - **Custom-provider chat history** now displays with its own platform identity instead of falling back to Claude branding
  - **Subscription webhook** no longer accidentally downgrades plan type during plan changes
...
<Update label="April 2026" description="Image Memories, Custom Platform Support, Pro → Lifetime Upgrade" tags={["Features", "Platform Support", "Improvements"]}>
  ...
  - **Describe it with Gemini Flash** so it's searchable by what's in the image
  - **Embed it with a multimodal model** so semantic image search works alongside your text memories
  - **Render thumbnails inline** in your memory list with an image badge
  - **Open in a lightbox** for full-size viewing with caption

  Image memories work everywhere your text memories do — search, MCP results (with signed URLs), bucket organization, and deletion. You can upload images directly from the dashboard's Add Memory dialog or from the browser extension panel.

  ### Custom platform chat history
  A new generic chat history upload endpoint lets third-party integrations and self-hosted platforms send conversations into MemoryPlugin in a standard format. Custom-platform conversations now appear with their own identity throughout the dashboard — the browse page, conversation list, and provider displays — instead of being lumped under an existing provider.

  ### Pro → Lifetime upgrade in settings
  Pro subscribers can now upgrade to a Lifetime plan directly from Settings. The page shows the correct upgrade price based on your existing subscription credit.

  ### Image lightbox polish
  - Wider caption area (up to `max-w-3xl`) with scrollable overflow
  - Cleaner close button cursor and caption handling
  - Image memories use a base64 multimodal embedding path for higher-quality results

  ### Extension v3.1.5 — Safari storage quota fix
  Safari was hitting its 5 MB default storage limit for users with heavy sync activity, causing thousands of background errors. The extension now requests `unlimitedStorage` permission so this no longer happens. Also fixed: the memory preload cache wasn't actually being read, so panels showed "Loading…" instead of cached memories instantly — now fixed.
  ...
  ### Reliability
  - **Self-healing chat imports** — a daily background job repairs conversations that imported without their messages
  - **Chat message ingestion failures** now surface as hard errors rather than silently completing
  - **Memory delete API** accepts both the internal DB ID and the public `pcId` UUID, so any client that has either identifier can delete a memory
  - **Vectorization race condition fixed** — deleted memories can no longer leave orphan entries in vector search
</Update>
...
<Update label="March 2026" description="Activity Analytics, New Platform Imports, Search Overhaul" tags={["Features", "Platform Support", "Improvements"]}>
  ...
  Each bucket now has its own knowledge graph view. MemoryPlugin extracts the key concepts across the memories in a bucket and visualizes how they connect — useful for seeing the shape of what you've collected on a topic. The renderer was rebuilt on Sigma.js for smoother interaction with larger graphs.

  ### AI-powered docs help on the dashboard
  A floating help button on the dashboard opens an in-app chat that answers questions about MemoryPlugin using the full documentation as context. Includes streaming responses, source citations as chips, follow-up question suggestions, and proper rendering of code blocks, tables, and headings — all without leaving the app.

  ### More reliable Monthly Insights
  Monthly Insights / Life Context generation moved to a dedicated background job runner with explicit status tracking, so the monthly summaries arrive more reliably and you can see when they're being prepared.

  ### OpenAI Apps Directory support
  Our MCP server now ships the tool annotations required for the OpenAI Apps Directory, so MemoryPlugin can show up as an app inside ChatGPT.
</Update>
