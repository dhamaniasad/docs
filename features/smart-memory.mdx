...
### In the Dashboard
The Smart Categories panel was redesigned to be easier to use. Once categories are created, you can:
...
- **Expand/collapse**: See more or less detail for each category
...
- **MCP Server**: Smart Memory categories are accessible through MCP-compatible clients via the `list_bucket_categories` and `list_category_memories` tools
