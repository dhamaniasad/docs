...
Export your chat history as JSON files from ChatGPT, Claude, TypingMind, or Grok and upload them to MemoryPlugin. This is the fastest way to import your entire history. Processing happens automatically in the background.

### Online Sync

For ChatGPT, Claude, Gemini, Grok, and DeepSeek, we can sync your conversations directly from your account without needing to download files. Designed for keeping new and updated conversations in sync automatically after your initial import. This handles small incremental updates in the background, so you never have to manually export again. Can take longer for large initial imports (several days vs. a few hours), but works as a fallback if you're unable to export JSON files.
...
You can import chat history from these platforms:

- **ChatGPT** (JSON export or online sync)
- **Claude** (JSON export or online sync)
- **Grok** (JSON export or online sync)
- **Gemini** (online sync)
- **DeepSeek** (online sync)
- **TypingMind** (JSON export)

<Info>
  We're continuing to expand import support to more platforms based on user
